...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderCart() {
    ...
    try {
      ...
      // ⭐️⭐️ الحل النهائي: التحقق من تسجيل الدخول والانتقال في نفس التبويب ⭐️⭐️
      checkoutBtn.addEventListener('click', (e) => {
          e.preventDefault(); // نضمن عدم الانتقال الافتراضي قبل التحقق
          ...
          if (isLoggedIn) {
              // إذا كان مسجلاً الدخول:
              // 1. حفظ السلة لضمان أن آخر تحديث قد تم قبل الانتقال
              localStorage.setItem('cart', JSON.stringify(cart));
              
              // 2. الأمر: فتح صفحة الشراء في نفس التبويب (window.location.href)
              window.location.href = 'Purchases/checkout.html'; 
              
              // 3. إغلاق قائمة السلة (في حالة عدم انتقال الصفحة بسرعة)
              ...
              alert('يجب عليك تسجيل الدخول أولاً لاستكمال عملية الشراء.');
              
              // (اختياري) يمكن توجيهه لصفحة الدخول هنا:
              // window.location.href = 'sign/sign.html?mode=login'; 
          }
      });
      // ⭐️⭐️ نهاية الحل النهائي ⭐️⭐️
  ...
  function addToCart(productId, qty = 1) {
    ...
    const price = parseFloat(product.dataset.price); // 🛠️ تم التعديل إلى parseFloat لدعم الأرقام العشرية
    ...
    if (!name || isNaN(price) || price <= 0) { // 🛠️ تم التحقق من أن السعر رقم صالح وموجب
      console.error('Invalid product data or price missing:', { productId, name, price, dataAttr: product.dataset.price });
      alert('عذراً، لا يمكن إضافة هذا المنتج. قد تكون بيانات سعره غير صحيحة أو مفقودة.');
      return;
    }
